...
#define GAMEMODENAME 	   "BY GITLINE"
#define VIP_TAG			   "R+"
//======================[ Конфигурация ТС ]======================//
...
public OnPlayerGiveDamage(playerid, damagedid, Float:amount, weaponid, bodypart) 
{
	...
	if(playerid != INVALID_PLAYER_ID) 
	{
		...
		if(PI[damagedid][pPassiveMode] == 1) 
		{
			if(IsPlayerInAnyVehicle(damagedid)) SetVehicleHealth(CarID, hpcar);
			SetPlayerHealthAC(damagedid, Health);
			SCM(playerid, COLOR_GREY, !"Урон заблокирован: у игрока включен пассивный режим");
			return 0;
		}
		if(PI[playerid][pPassiveMode] == 1)
		{
			if(IsPlayerInAnyVehicle(damagedid)) SetVehicleHealth(CarID, hpcar);
			SetPlayerHealthAC(damagedid, Health);
			SCM(playerid, COLOR_GREY, !"Урон заблокирован: у Вас включен пассивный режим");
			return 0;
		}	
			...
			if(IsPlayerInRangeOfPoint(playerid, 75.0, 1616.7029,-1238.1493,19.3756) && IsPlayerInRangeOfPoint(killerid, 75.0, 1616.7029,-1238.1493,19.3756)) 
			{
				if(PI[playerid][pMember] == Command[0])  
				{
					if(PI[killerid][pMember] == Command[1]) 
					{
						...
						format(killer, 32, "{%s}%s[%d]", ColorTeam[PI[killerid][pMember]], getName(killerid), killerid); // убийца
						format(killed, 32, "{%s}%s[%d]", ColorTeam[PI[playerid][pMember]], getName(playerid), playerid); // убитый
	...
	switch(dialogid) 
	{
		...
		case 3298:
		{
			...
			if(response)
			{
				...
				switch(listitem)
				{
					case 0:
					{
						return SendR(playerid);//SendRequestForPlayer(playerid, targplayer[0], 4);
					}
					case 1:
					{
						if(PI[targplayer[0]][pHealPack] >= 3) return SCM(playerid, COLOR_GREY, !"У игрока уже есть 3 набора аптечек");
						return SendR(playerid);//SendRequestForPlayer(playerid, targplayer[0], 2);
					}
					case 2:
					{
						return SendR(playerid);//SendRequestForPlayer(playerid, targplayer[0], 9);
					}
					case 3:
					{
						return SendR(playerid);//SendRequestForPlayer(playerid, targplayer[0], 7);
					}
					case 4:
					{
						return SendR(playerid);//SendRequestForPlayer(playerid, targplayer[0], 6);
					}
					case 5:
					{
						return SendR(playerid);//SendRequestForPlayer(playerid, targplayer[0], 10);
					}
					case 6:
					{
						if(PI[targplayer[0]][data_MASK] >= 1) return SCM(playerid, COLOR_GREY, !"У игрока уже есть маска");
						return SendR(playerid);//SendRequestForPlayer(playerid, targplayer[0], 3);
					}
				}
			}
		}
		...
		/*case 9221:
		{
			...
			if(response) 
			{
				if(ProxDetectorS(10.0, playerid, from_player))
				{
					...
					UpdatePlayerDataInt(from_player, "Money", from_player);
					//==========================================================
		...
		/*case 9226:
		{
			...
			if(response) 
			{
				...
				if(PI[id][pRequestFrom] != -1 && PI[id][pRequestIDFor] != -1) 
				...
				if(CarInfo[PI[playerid][pLoadVehicleID]][cPremium] == 1) 
				{
					...
					SendR(playerid);//SendRequestForPlayer(playerid, id, 1, cost, CarInfo[PI[playerid][pLoadVehicleID]][cCost]/100*20);
				}
				else
				{
					...
					SendR(playerid);//SendRequestForPlayer(playerid, id, 1, cost, CarInfo[PI[playerid][pLoadVehicleID]][cCost]/100*20); // отправляем игроку что ему кинули приглос на покупку
				}
				return 1;
			}
		}*/
...
public OnPlayerSpawn(playerid)
{
	...
	CreateObject(9421, 2050.36, 1734.23, -44.03,   0.00, 0.00, 0.00);
	CreateObject(9423, 2050.36, 1734.23, -44.03,   0.00, 0.00, 0.00);
	CreateObject(9423, 2050.36, 1734.23, -44.03,   0.00, 0.00, 0.00);
	CreateObject(9435, 2056.81, 1635.59, -44.05,   0.00, 0.00, 0.00);
	CreateObject(9436, 2056.81, 1635.59, -44.05,   0.00, 0.00, 0.00);
	CreateObject(9437, 2056.81, 1635.59, -44.05,   0.00, 0.00, 0.00);
	CreateObject(9438, 2068.55, 1654.46, -44.05,   0.00, 0.00, 0.00);
	CreateObject(9439, 2072.28, 1632.34, -45.50,   0.00, 0.00, 0.00);
	CreateObject(9440, 2075.34, 1628.17, -45.52,   0.00, 0.00, 22.50);
	CreateObject(9441, 2072.01, 1625.76, -45.52,   0.00, 0.00, -52.56);
	CreateObject(9442, 2078.14, 1624.71, -45.52,   0.00, 0.00, -78.36);
	CreateObject(9442, 2081.50, 1622.35, -45.52,   0.00, 0.00, -78.36);
	CreateObject(9441, 2076.48, 1620.29, -45.36,   0.00, 0.00, -11.04);
	CreateObject(9439, 2074.61, 1620.37, -45.36,   0.00, 0.00, -42.78);
	CreateObject(9440, 2069.61, 1619.96, -45.36,   0.00, 0.00, -105.12);
	CreateObject(9440, 2070.83, 1636.65, -45.35,   0.00, 0.00, 69.54);
	CreateObject(9440, 2066.03, 1631.37, -45.35,   0.00, 0.00, 110.70);
	CreateObject(9441, 2065.42, 1633.86, -45.35,   0.00, 0.00, 151.62);
	CreateObject(9442, 2067.10, 1639.80, -45.35,   0.00, 0.00, 96.54);
	CreateObject(9443, 2125.58, 1577.36, -45.31,   0.00, 0.00, 0.00);
	CreateObject(9445, 2125.73, 1578.63, -45.71,   0.00, 0.00, 0.00);
	CreateObject(9446, 2125.73, 1578.63, -45.71,   0.00, 0.00, 0.00);
	CreateObject(9447, 2125.73, 1578.63, -45.71,   0.00, 0.00, 0.00);
	CreateObject(9448, 2125.73, 1578.63, -45.71,   0.00, 0.00, 0.00);
	CreateObject(9449, 2125.69, 1578.59, -45.71,   0.00, 0.00, 0.00);
	CreateObject(9450, 2125.71, 1578.60, -45.71,   0.00, 0.00, 0.00);
	CreateObject(9451, 2125.71, 1578.60, -45.71,   0.00, 0.00, 0.00);
	CreateObject(9452, 2125.71, 1578.60, -45.71,   0.00, 0.00, 0.00);
	CreateObject(9453, 2125.71, 1578.60, -45.71,   0.00, 0.00, 0.00);
	CreateObject(9453, 2125.71, 1578.60, -45.71,   0.00, 0.00, 0.00);
	CreateObject(9454, 2125.71, 1578.60, -45.71,   0.00, 0.00, 0.00);
	CreateObject(9444, 2125.61, 1577.48, -46.13,   0.00, 0.00, 0.00);
	CreateObject(9456, 2095.63, 1561.76, -46.26,   0.00, 0.00, 0.00);
	CreateObject(9457, 2094.23, 1562.33, -47.39,   0.00, 0.00, 0.00);
	CreateObject(9458, 2097.11, 1563.09, -46.54,   0.00, 0.00, 0.00);
	CreateObject(9459, 2094.55, 1563.14, -46.54,   0.00, 0.00, 0.00);
	CreateObject(9460, 2095.54, 1566.24, -46.27,   0.00, 0.00, 0.00);
	CreateObject(9461, 2097.09, 1566.52, -47.37,   0.00, 0.00, 0.00);
	CreateObject(9462, 2095.75, 1568.32, -46.49,   0.00, 0.00, 0.00);
	CreateObject(9463, 2095.76, 1567.89, -48.36,   0.00, 0.00, 0.00);
	CreateObject(9465, 2098.14, 1567.66, -48.87,   0.00, 180.00, 0.00);
	CreateObject(9465, 2098.14, 1567.66, -48.87,   0.00, 180.00, 0.00);
	CreateObject(9468, 1799.74, 808.71, 16.23,   0.00, 0.00, 92.58);
	CreateObject(9469, 1799.74, 808.71, 16.23,   0.00, 0.00, 92.58);
	CreateObject(9470, 2146.56, 1535.94, -46.81,   0.00, 0.00, 0.00);
	CreateObject(9471, 2120.12, 1494.60, -46.81,   0.00, 0.00, 0.00);
	CreateObject(9472, 2141.29, 1494.75, -46.81,   0.00, 0.00, 0.00);
	CreateObject(9473, 2081.19, 1509.61, -46.81,   0.00, 0.00, 0.00);
	CreateObject(9474, 2085.87, 1497.64, -46.77,   0.00, 0.00, 0.00);
	CreateObject(9475, 2105.51, 1459.21, -46.77,   0.00, 0.00, 0.00);
	CreateObject(9476, 2072.07, 1459.01, -46.77,   0.00, 0.00, 0.00);
	CreateObject(9477, 2147.17, 1536.84, -46.55,   0.00, 0.00, 0.00);
	CreateObject(9478, 2128.80, 1494.59, -46.81,   0.00, 0.00, 0.00);
	CreateObject(9478, 2128.81, 1494.59, -46.81,   0.00, 0.00, 180.46);
	CreateObject(9479, 2128.82, 1493.92, -48.56,   0.00, 0.00, 180.46);
	CreateObject(9480, 2131.86, 1497.26, -48.56,   0.00, 0.00, 265.60);
	CreateObject(9480, 2123.25, 1497.24, -48.56,   0.00, 0.00, 265.60);
	CreateObject(9480, 2124.66, 1491.94, -48.56,   0.00, 0.00, 443.38);
	CreateObject(9480, 2116.00, 1492.00, -48.56,   0.00, 0.00, 443.38);
...
CMD:card(playerid,params[]) 
{
	...
	return SendR(playerid);//SendRequestForPlayer(playerid, params[0], 6);
}
CMD:lic(playerid,params[]) 
{
    if(sscanf(params, "u", params[0])) 
	{
		...
		Гражданин РФ\t\t\t{3377cc}%s\n\
		%s\n\
		%s", PI[playerid][pLicNumber], getName(playerid), licb, licg);
	}
	if(!IsPlayerConnected(params[0])) return SCM(playerid, COLOR_LIGHTGREY, !"Используйте: /lic [ид игрока]");
	if(!IsPlayerLogged{params[0]}) return SCM(playerid, COLOR_GREY, !"Игрок не авторизован");
	if(!ProxDetectorS(10.0, playerid, params[0])) return SCM(playerid, COLOR_GREY, !"Игрок слишком далеко от Вас");
	return SendR(playerid);//SendRequestForPlayer(playerid, params[0], 7);
}
CMD:dice(playerid,params[]) 
{
	...
	return SendR(playerid);//SendRequestForPlayer(playerid, params[0], 8, params[1]);
}
CMD:pass(playerid,params[]) 
{
	...
	GetPlayerPos(params[0],x,y,z);
	if(!ProxDetectorS(10.0, playerid, params[0])) return SCM(playerid, COLOR_GREY, !"Игрок находится слишком далеко");
	return SendR(playerid);//SendRequestForPlayer(playerid, params[0], 9);
}
CMD:skill(playerid,params[]) 
{
	new str_3[365];
    if(sscanf(params, "u", params[0])) {
		format(str_3,sizeof(str_3),"1. ТТ:\t\t\t%d %%\n2. Тайзер:\t%d %%\n3. ПЛ-15:\t\t%d %%\n4. Дробовик:\t\t\t%d %%\n5. MP5:\t\t\t%d %%\n6. AK-47:\t\t%d %%",PI[playerid][pSkillPistol],PI[playerid][pSkillSDPistol],PI[playerid][pSkillDeagle],PI[playerid][pSkillShotgun],PI[playerid][pSkillMP5],PI[playerid][pSkillAK47]);
...
{
	return ShowPlayerDialog(playerid, 7515, DIALOG_STYLE_LIST, !"{ff0000}Меню Государственной волны", !"{0f0085}1. Запланировать за 30 минут\n\
	...
	{0f0085}7. Отменить собеседование/призыв", !"Выбрать", !"Отмена");
}
stock open_govvh(playerid) 
{
	return ShowPlayerDialog(playerid, 7788, DIALOG_STYLE_LIST, !"{ff0000}Меню Государственной волны", !"{0f0085}1. Запланировать за 30 минут\n\
	{0f0085}2. Запланировать за 15 минут\n\
	{0f0085}3. Запланировать за 5 минут\n\
 	{0f0085}4. Начать собеседование/призыв\n\
	{0f0085}5. Продолжить призыв\n\
	{0f0085}6. Закончить призыв\n\
	{0f0085}7. Отменить призыв", !"Выбрать", !"Отмена");
}
cmd:gov(playerid) 
{
    if(PI[playerid][pMute] == 1) return SCM(playerid, COLOR_GREY, !"Ваш чат временно заблокирован");
...
cmd:free(playerid, params[]) 
{
    if(PI[playerid][pMember] == 1 && PI[playerid][pRang] == 5) 
	{
		...
		SendR(playerid);//SendRequestForPlayer(playerid, params[0], 12, GetPVarInt(params[0],"free_money"));
    }
...
CMD:givelic(playerid,params[]) 
{
    if(PI[playerid][pMember] == 1 && PI[playerid][pRang] == 4) 
	{
		...
		if(params[1] == 1) 
		{
			...
			SendR(playerid);//SendRequestForPlayer(playerid, params[0], 14);
		}
		else 
		{
		    if(PI[params[0]][pGunLicense] == 1) return SCM(playerid, COLOR_GREY, !"У игрока уже есть лицензия на оружие");
		    SendR(playerid);//SendRequestForPlayer(playerid, params[0], 13);
		}
	}
	else SCM(playerid, COLOR_GREY, !"Данная команда Вам недоступна");
	return 1;
...
CMD:heal(playerid,params[]) 
{
	...
	GetPlayerPos(params[0],x,y,z);
	if(!PlayerToPoint(10.0, playerid, x,y,z)) return SCM(playerid, COLOR_GREY, !"Игрок находится слишком далеко");
	return SendR(playerid);//SendRequestForPlayer(playerid, params[0], 17);
}
...
CMD:wanted(playerid)
{
    ...
    if (bugfix == 0) 
        return ShowPlayerDialog(playerid, 0, DIALOG_STYLE_LIST, "{ee3366}Преступники", "Область чиста, преступники не обнаружены!", "Закрыть", "");
...
CMD:givemask(playerid,params[]) 
{
	...
	GetPlayerPos(params[0],x,y,z);
	if(!PlayerToPoint(10.0, playerid, x,y,z)) return SCM(playerid, COLOR_GREY, !"Игрок слишком далеко от Вас");
	SendR(playerid);//SendRequestForPlayer(playerid, params[0], 3);
	return 1;
}
...
CMD:givegun(playerid, params[]) 
{
	...
	return SendR(playerid);//SendRequestForPlayer(playerid, params[0], 16, params[1]);
}
...
callback: ShowLeaders(playerid)
{
    ...
    global_str[0] = EOS, format(global_str, sizeof(global_str), "Организация\tДолжность\tИмя\tТелефон\tСостояние\n%s", string);
