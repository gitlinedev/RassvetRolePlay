			...
			if(response)
   			{
				switch(listitem)
				{
				    case 0: 
					{
						ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, !"{ee3366}Команды игрового модератора", !"\
					...
					case 1: 
					{
						...
						giveskill - выдать навыки стрельбы игроку", !"Закрыть", !"");
					}
					case 2: 
					{
						ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, !"{ee3366}Команды игрового модератора", !"\
...
CMD:mhelp(playerid) 
{
	...
	return ShowPlayerDialog(playerid, 2840, DIALOG_STYLE_LIST, "{ee3366}Команды игрового модератора", dialog, "Выбрать", "Закрыть");
}